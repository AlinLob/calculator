...
const handleDigit = (key) => {
  ...
  if (currentValue.includes('.') && currentValue.split('.')[1].length >= 15) {
    currentValue = (parseFloat(currentValue)).toFixed(15);
  } else if (currentValue.length < 30) { // Ограничение символов перед точкой
    currentValue += key;
  }
